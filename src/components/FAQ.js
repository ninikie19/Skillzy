    ...
    return (<div className="py-12 bg-gray-50 dark:bg-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          {faqs.map(function (faq, index) { return (<div key={index} className="bg-white dark:bg-gray-700 rounded-lg shadow-md overflow-hidden">
              <button className="w-full flex justify-between items-center p-6 text-left" onClick={function () { return toggleFAQ(index); }}>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                  {faq.question}
                </h3>
                <svg className={"h-6 w-6 text-gray-500 dark:text-gray-400 transform transition-transform ".concat(openIndex === index ? 'rotate-180' : '')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={openIndex === index ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"}/>
                </svg>
              </button>
              {openIndex === index && (<div className="px-6 pb-6 text-gray-600 dark:text-gray-300">
                  {faq.answer}
                </div>)}
            </div>); })}
        </div>
